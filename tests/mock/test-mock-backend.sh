...
test_output_streaming() {
    ...
    local output=$(RALPH_MOCK_DELAY=1 RALPH_MOCK_RESPONSE=progress "$MOCK_OPENCODE" run --agent test 2>&1)
...
test_error_handling_invalid_command() {
    ...
    local output=$("$MOCK_OPENCODE" invalid-command 2>&1) || local exit_code=$?
...
test_error_handling_missing_args() {
    ...
    local output=$("$MOCK_OPENCODE" test 2>&1) || local exit_code=$?
...
test_integration_single_iteration() {
    ...
    assert_contains "$output" "RalphLoop Iteration 1 of 1" "Should show single iteration"
